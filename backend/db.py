...
SQLITE = "SQLITE"
DB_TYPE = SQLITE
database_path = os.path.abspath("ovs.sqlite3")
...
class Database(Compat):
    def __init__(self, db_name: str):
        ...
        if DB_TYPE == SQLITE:
            self.path = os.path.join(f"{self.name}.sqlite3")
        ...
        self.engine = create_engine(database_uri, strategy=ASYNCIO_STRATEGY, echo=False)
    ...
    async def clean_ext_db_files(cls, ext_id: str) -> bool:
        ...
        if DB_TYPE == SQLITE:
            db_file = os.path.join(f"ext_{ext_id}.sqlite3")
